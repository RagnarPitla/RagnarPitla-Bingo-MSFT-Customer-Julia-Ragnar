...
export function AnimatedPerson({
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setBobOffset(Math.sin(Date.now() / 1000 + index * 1.8) * 3);
    }, 50);
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      {isCurrentUser && (
        <div
          className="z-20 mb-0.5"
          style={{ filter: "drop-shadow(0 0 5px hsl(45, 100%, 65%)) drop-shadow(0 0 2px hsl(35, 80%, 40%))" }}
        >
          <svg width="24" height="15" viewBox="0 0 24 15" fill="none">
            <path d="M1 14 L1 8.5 L6 2 L12 7.5 L18 2 L23 8.5 L23 14 Z" fill="hsl(45, 95%, 52%)" stroke="hsl(35, 65%, 35%)" strokeWidth="0.7" strokeLinejoin="round" />
      ...
      {/* Avatar — bobs up and down */}
      <div style={{ transform: `translateY(${bobOffset}px)`, transition: "transform 0.05s" }}>

        {/* Gold glow for current user */}
        {isCurrentUser && (
          <div className="absolute -inset-2 rounded-full animate-pulse opacity-60 pointer-events-none"
            style={{ background: "radial-gradient(circle, hsl(45, 100%, 60%, 0.35), transparent 70%)" }}
          />
        )}

        <svg width="52" height="60" viewBox="0 0 48 56" fill="none" className="md:w-14 md:h-16 drop-shadow-lg relative">
          <rect x="18" y="38" width="12" height="8" rx="3" fill={colors.skin} />
          <path d="M8 52 C8 46, 16 42, 24 42 C32 42, 40 46, 40 52 L40 56 L8 56 Z"
            fill={role === "dealer" ? "hsl(0, 0%, 95%)" : colors.shirt}
            stroke={role === "dealer" ? "hsl(0, 0%, 80%)" : colors.shirt}
            strokeWidth="0.5"
          />
          {role === "dealer" && (
            <>
          ...
          <path d="M10 20 C10 8, 18 3, 24 3 C30 3, 38 8, 38 20 C38 14, 34 9, 24 9 C14 9, 10 14, 10 20 Z" fill={colors.hair} />
          ...
          <path d="M23 25 Q24 27, 25 25" stroke={colors.skin} strokeWidth="0.8" fill="none" />
          <path d="M19 30 Q24 34, 29 30" stroke="hsl(0, 30%, 45%)" strokeWidth="1" fill="none" strokeLinecap="round" />
        ...
        {/* Name tag */}
        <div
          className="mt-0.5 text-center px-2 py-0.5 rounded-md max-w-[80px]"
          style={{
            background: isCurrentUser ? "hsl(45, 95%, 52%, 0.12)" : "hsl(222, 40%, 12%, 0.85)",
            border: isCurrentUser ? "1px solid hsl(45, 95%, 52%, 0.5)" : "1px solid hsl(222, 30%, 22%)",
          }}
        >
          {isCurrentUser && (
            <p className="text-[6px] font-bold uppercase tracking-wider leading-tight" style={{ color: "hsl(45, 95%, 60%)" }}>You</p>
          )}
          <p className="text-[9px] font-semibold text-foreground truncate leading-tight">{name}</p>
          <p className="text-[7px] text-muted-foreground truncate leading-tight">{company}</p>
        </div>
      ...
      {/* Stool — stays fixed while avatar bobs */}
      <svg width="64" height="48" viewBox="0 0 64 48" fill="none" className="mt-1">
        {/* Seat */}
        <ellipse cx="32" cy="10" rx="28" ry="9" fill="#111" stroke="#2a2a2a" strokeWidth="1.5" />
        <ellipse cx="32" cy="8" rx="22" ry="6" fill="#0a0a0a" />
        {/* Highlight */}
        <path d="M12 7 Q20 3, 30 6" stroke="#2a2a2a" strokeWidth="1" fill="none" strokeLinecap="round" />
        {/* Pole */}
        <rect x="28" y="17" width="8" height="22" rx="3" fill="#161616" stroke="#222" strokeWidth="0.5" />
        {/* Base */}
        <ellipse cx="32" cy="41" rx="24" ry="6" fill="#111" stroke="#222" strokeWidth="1" />
      </svg>
    </div>
  );
}
